...
?>">
	...
	if ( has_post_thumbnail() && (empty($args['featured']) || $args['featured']=='image')) {
		...
		?><a href="<?php echo esc_url($link); ?>"
			 id="<?php echo esc_attr($args['id'].'_'.trim(str_replace(' ', '_', $meta['icon'])).'_'.rand(0,999999)); ?>"
			 class="sc_services_item_icon <?php echo empty($svg) ? $meta['icon'] == "No icon" ? "icon-untitled-9" : esc_attr($meta['icon']) : 'sc_icon_type_svg'; ?>"
			 ><?php
			if (!empty($svg)) {
	...
	<div class="sc_services_item_info">
		...
		<?php } ?>
        <a class="sc_services_item_link" href="<?php echo esc_url($link); ?>"><?php esc_html_e('Learn More', 'snowmountain'); ?></a>
	</div>
</div>
<?php
