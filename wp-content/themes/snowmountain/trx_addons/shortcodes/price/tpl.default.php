...
?>><?php
	...
	if (!empty($args['image'])) {
		...
		?><img src="<?php echo esc_url($args['image']); ?>" alt="<?php esc_attr__('Image', 'snowmountain')?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>><?php
        if (!empty($args['title'])) {
	...
	?><div class="sc_price_info"><?php
		...
		if (!empty($args['subtitle'])) {
			...
			?><div class="sc_price_price"><?php
				...
				?><span class="sc_price_value"><?php echo wp_kses($parts[0], 'snowmountain_kses_content' ); ?></span><?php
				if (count($parts) > 1 && $parts[1]!='') {
					?><span class="sc_price_decimals"><?php echo wp_kses($parts[1], 'snowmountain_kses_content'); ?></span><?php
				}
			?></div><?php
		}

		if (!empty($args['description'])) {
			?><div class="sc_price_description"><?php echo wp_kses(trx_addons_parse_codes($args['description']), 'snowmountain_kses_content' ); ?></div><?php
		}
