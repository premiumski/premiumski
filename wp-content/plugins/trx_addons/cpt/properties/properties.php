	...
	function trx_addons_cpt_properties_init() {
		...
		$TRX_ADDONS_STORAGE['meta_box_'.TRX_ADDONS_CPT_PROPERTIES_PT] = array(
			"basic_section" => array(
				...
				"desc" => wp_kses_data( __('Basic information of the property', 'trx_addons') ),
				"type" => "section"
			),
			"before_price" => array(
...
if ( !function_exists( 'trx_addons_cpt_properties_query_params_to_args' ) ) {
	function trx_addons_cpt_properties_query_params_to_args($params=array(), $new_query=false) {
		...
		else if (!empty($params['properties_bedrooms']))
		...
		else if (!empty($params['properties_bathrooms']))
		...
		else if (!empty($params['properties_area']))
		...
		else if (!empty($params['properties_price']))
		...
		else if (!empty($params['properties_keyword']))
			$keywords = array(
				'relation' => 'OR',
				array(
		...
		else if (!empty($params['properties_order'])) {
				...
				array(
					array(
						...
						"value" => apply_filters('trx_addons_sc_type', array(
							...
							esc_html__('Slider', 'trx_addons') => 'slider',
							esc_html__('Google map', 'trx_addons') => 'googlemap'
						), 'trx_sc_properties' ),
						"type" => "dropdown"
					),
					array(
