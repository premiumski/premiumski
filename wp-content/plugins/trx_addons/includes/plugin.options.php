...
if (!function_exists('trx_addons_init_options')) {
	...
	function trx_addons_init_options() {
		...
		$TRX_ADDONS_STORAGE['options'] = apply_filters('trx_addons_filter_options', array(
			...
			// Section 'Internal plugin settings'
			'socials_type' => array(
				"title" => esc_html__('Type of socials icons', 'trx_addons'),
				"desc" => esc_html__('images|icons - Use images or icons as pictograms of the social networks', 'trx_addons'),
				"std" => 'icons',
				"type" => "hidden"
			),
			'icons_selector' => array(
				"title" => esc_html__('Type of icons selector', 'trx_addons'),
				"desc" => esc_html__('vc|internal - Use standard VC parameters for icons or use internal popup with theme icons', 'trx_addons'),
				"std" => 'vc',
				"type" => "hidden"
			)
...
if (!function_exists('trx_addons_before_show_options')) {
	...
	function trx_addons_before_show_options($options, $post_type, $group='') {
		...
		foreach ($options as $id=>$field) {
			...
			if ((in_array($group, array('socials', 'share')) && $id=='name') || $field['type']=='icons') {
				// Icons list
				if ((!empty($field['style']) && $field['style']=='icons') || trx_addons_get_option('socials_type')=='icons') {
					if ($icons_list === false) $icons_list = trx_addons_get_list_icons();
					$options[$id]['options'] = $icons_list;
					
				// Images list
				} else {
					if ($images_list === false) $images_list = trx_addons_get_list_files('css/socials', 'png');
